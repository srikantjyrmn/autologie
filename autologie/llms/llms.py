...
from .response_handlers_agent import (
...
class AgentClient:
    ...
    def __init__(
        ...
        self.state = 'idle'
        self.states = ['idle', 'message_received', 'api_response_received', 'api_response_validated', 'send_message_to_user', 'get_feedback_from_self']
    ...
    def listen(self, message:str):
        self.messages.append(UserMessage(content = message))
        self.state = 'user_message_received'
        self.validated = False

    def speak(self):
        self.inference_logger.info(f"Speaking, Inference Count: {self.inference_count}")
        self.api.run_unvalidated_inference(self.messages)
        api_response = self.api.run_unvalidated_inference(
            messages = self.messages,
            response_format= self.llm_spec.response_format,
            tools=self.llm_spec.tools
    ...
    def reflect(self):
        n_tries = 5
        self.inference_logger.info("Reflecting")
        while not self.validated:
            ...
            self.inference_logger.info(f"***Reflection {5-n_tries} :Got validation object {completion_validation}")
            n_tries -= 1

            # If validated, return response
            if (completion_validation.validation):
                if not completion_validation.get_feedback:
                    self.validated = True
                    self.inference_logger.info(f"\n***Relfection: Got Validated Response: {completion_validation}. Returning \n")
                return completion_validation#, api_response, tool_calls
            # If not validated, continue loop
            #print(f"Object validation failed. {n_tries} tries left. Trying again.\n\n\n\n\n\n {completion_validation.assistant_message} \n\n\n\n")
            self.messages.append(ToolMessage(content = completion_validation.assistant_message))
            continue
        # If n_tries is exceeded, returns the last available response
        #self.completion_validation = completion_validation
        return completion_validation

    def act(self, completion_validation):
        self.inference_logger.info("Acting")
        if self.validated:
            if not completion_validation.get_feedback:
